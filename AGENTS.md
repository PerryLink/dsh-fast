...
- `test/` — vitest; REAL `Context`/`SessionStore`/`Session`/`ToolRuntime`/`CommandRuntime`/`TokenMeter` and the REAL storage seam (dsh-storage + dsh-storage-json + dsh-storage-domain) from the 0.1.1-rc.2 peers. Only the optional token-meter measure function is scripted.
